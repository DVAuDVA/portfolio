...
namespace ChatBot.Commands
{
 ...
 c.Trigger.Equals(cmd,
 StringComparison.OrdinalIgnoreCase));
                
                if (command != null)
                {
                    _logger.LogInformation("Выполняется команда {Command}",
 cmd);
                ...
                {
                    _logger.LogWarning("Неизвестная команда: {Command}",
 cmd);
            ...
            var history
 = await _chatModelRepository.GetHistoryAsync(chatId);
            try
            {
                ...
                var result = await _chatClient.SendMessageAsync(text,
 history);
